...
/**
 * A TitledPane that supports localizing the name (id), style, text, graphic, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulTitledPane extends TitledPane implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTitledPane with the specified Resource.
     * @param resource A Resource containing a TitledPanePropertyBundle.
     * @return A ResourcefulTitledPane with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTitledPane create(Resource resource)
    {
        ResourcefulTitledPane titledPane = new ResourcefulTitledPane(resource);
        titledPane.initialize();
        return titledPane;
    }
    ...
    /**
     * Constructs a TitledPane bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulTitledPane(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            TitledPanePropertyBundle props = (TitledPanePropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.titled.pane", ex.getMessage(), ex);
        }
    }
