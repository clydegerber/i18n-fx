...
/**
 * A Label that supports localizing the name (id), style, text, graphic, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulLabel extends Label implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulLabel with the specified Resource.
     * @param resource A Resource containing a LabeledPropertyBundle.
     * @return A ResourcefulLabel with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulLabel create(Resource resource)
    {
        ResourcefulLabel label = new ResourcefulLabel(resource);
        label.initialize();
        return label;
    }
    ...
    /**
     * Constructs a Label bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulLabel(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            LabeledPropertyBundle props = (LabeledPropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.label", ex.getMessage(), ex);
        }
    }
