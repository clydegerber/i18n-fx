...
/**
 * A CheckBox that supports localizing the name (id), style, text, graphic, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulCheckBox extends CheckBox implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulCheckBox with the specified Resource.
     * @param resource A Resource containing a ButtonBasePropertyBundle.
     * @return A ResourcefulCheckBox with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulCheckBox create(Resource resource)
    {
        ResourcefulCheckBox checkBox = new ResourcefulCheckBox(resource);
        checkBox.initialize();
        return checkBox;
    }
    ...
    /**
     * Constructs a CheckBox bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulCheckBox(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.check.box", ex.getMessage(), ex);
        }
    }
