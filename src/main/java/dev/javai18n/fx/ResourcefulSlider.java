...
/**
 * A Slider that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulSlider extends Slider implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulSlider with the specified Resource.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulSlider with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulSlider create(Resource resource)
    {
        ResourcefulSlider slider = new ResourcefulSlider(resource);
        slider.initialize();
        return slider;
    }
    ...
    /**
     * Constructs a Slider bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulSlider(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.slider", ex.getMessage(), ex);
        }
    }
