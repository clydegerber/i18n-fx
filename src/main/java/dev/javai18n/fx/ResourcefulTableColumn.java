...
public class ResourcefulTableColumn<S,T> extends TableColumn<S,T> implements Resourceful, LocaleEventListener
{
     ...
     */
    public static <S,T> ResourcefulTableColumn<S,T> create(Resource resource)
    {
        ResourcefulTableColumn<S,T> col = new ResourcefulTableColumn<>(resource);
        col.initialize();
        return col;
    }
     ...
     */
    public ResourcefulTableColumn()
    {
        this.delegate = new FXResourcefulDelegate(this::updateLocaleSpecificValues);
        tableViewProperty().addListener(new javafx.beans.value.ChangeListener<javafx.scene.control.TableView<S>>()
        {
            @Override
            public void changed(javafx.beans.value.ObservableValue<? extends javafx.scene.control.TableView<S>> obs,
                                javafx.scene.control.TableView<S> old, javafx.scene.control.TableView<S> tv)
            {
                if (tv != null)
                {
                    if (!delegate.isInitialized())
                        throw new IllegalStateException(
                            "ResourcefulTableColumn added to TableView before initialize(Resource) was called");
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            TableColumnPropertyBundle props = (TableColumnPropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.table.column", ex.getMessage(), ex);
        }
    }
