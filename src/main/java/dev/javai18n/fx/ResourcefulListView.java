...
/**
 * A ListView that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 *
 * @param <E> The item type held by the list.
 */
public class ResourcefulListView<E> extends ListView<E> implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulListView with the specified Resource.
     * @param <E> The item type held by the list.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulListView with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static <E> ResourcefulListView<E> create(Resource resource)
    {
        ResourcefulListView<E> listView = new ResourcefulListView<>(resource);
        listView.initialize();
        return listView;
    }
    ...
    /**
     * Constructs a ListView bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulListView(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.list.view", ex.getMessage(), ex);
        }
    }
