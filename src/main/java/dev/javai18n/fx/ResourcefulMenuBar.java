...
/**
 * A MenuBar that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulMenuBar extends MenuBar implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulMenuBar with the specified Resource.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulMenuBar with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulMenuBar create(Resource resource)
    {
        ResourcefulMenuBar menuBar = new ResourcefulMenuBar(resource);
        menuBar.initialize();
        return menuBar;
    }
    ...
    /**
     * Constructs a MenuBar bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulMenuBar(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.menu.bar", ex.getMessage(), ex);
        }
    }
