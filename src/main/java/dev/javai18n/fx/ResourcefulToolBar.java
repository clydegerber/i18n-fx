...
/**
 * A ToolBar that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulToolBar extends ToolBar implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulToolBar with the specified Resource.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulToolBar with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulToolBar create(Resource resource)
    {
        ResourcefulToolBar toolBar = new ResourcefulToolBar(resource);
        toolBar.initialize();
        return toolBar;
    }
    ...
    /**
     * Constructs a ToolBar bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulToolBar(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.tool.bar", ex.getMessage(), ex);
        }
    }
