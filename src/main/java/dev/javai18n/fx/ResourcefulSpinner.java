...
/**
 * A Spinner that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 *
 * @param <T> The type of the spinner value.
 */
public class ResourcefulSpinner<T> extends Spinner<T> implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulSpinner with the specified Resource.
     * @param <T> The type of the spinner value.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulSpinner with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static <T> ResourcefulSpinner<T> create(Resource resource)
    {
        ResourcefulSpinner<T> spinner = new ResourcefulSpinner<>(resource);
        spinner.initialize();
        return spinner;
    }
    ...
    /**
     * Constructs a Spinner bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulSpinner(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.spinner", ex.getMessage(), ex);
        }
    }
