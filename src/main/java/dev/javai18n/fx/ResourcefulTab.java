 ...
 */
public class ResourcefulTab extends Tab implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTab with the specified Resource.
     *
     * @param resource A Resource containing a TabPropertyBundle.
     * @return A ResourcefulTab with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTab create(Resource resource)
    {
        ResourcefulTab tab = new ResourcefulTab(resource);
        tab.initialize();
        return tab;
    }
     ...
     */
    public ResourcefulTab()
    {
        this.delegate = new FXResourcefulDelegate(this::updateLocaleSpecificValues);
        tabPaneProperty().addListener(new javafx.beans.value.ChangeListener<javafx.scene.control.TabPane>()
        {
            @Override
            public void changed(javafx.beans.value.ObservableValue<? extends javafx.scene.control.TabPane> obs,
                                javafx.scene.control.TabPane old, javafx.scene.control.TabPane tp)
            {
                if (tp != null)
                {
                    if (!delegate.isInitialized())
                        throw new IllegalStateException(
                            "ResourcefulTab added to TabPane before initialize(Resource) was called");
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            TabPropertyBundle props = (TabPropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.tab", ex.getMessage(), ex);
        }
    }
