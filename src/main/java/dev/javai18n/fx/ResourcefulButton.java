...
/**
 * A Button that supports localizing the name (id), style, text, graphic, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulButton extends Button implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulButton with the specified Resource.
     * @param resource A Resource containing a ButtonBasePropertyBundle.
     * @return A ResourcefulButton with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulButton create(Resource resource)
    {
        ResourcefulButton button = new ResourcefulButton(resource);
        button.initialize();
        return button;
    }
    ...
    /**
     * Constructs a Button bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulButton(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.button", ex.getMessage(), ex);
        }
    }
