...
/**
 * A SplitPane that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulSplitPane extends SplitPane implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulSplitPane with the specified Resource.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulSplitPane with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulSplitPane create(Resource resource)
    {
        ResourcefulSplitPane splitPane = new ResourcefulSplitPane(resource);
    ...
    /**
     * Constructs a SplitPane bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulSplitPane(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.split.pane", ex.getMessage(), ex);
        }
    }
