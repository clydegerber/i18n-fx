...
/**
 * A TextField that supports localizing the name (id), style, prompt text, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulTextField extends TextField implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTextField with the specified Resource.
     * @param resource A Resource containing a TextInputPropertyBundle.
     * @return A ResourcefulTextField with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTextField create(Resource resource)
    {
        ResourcefulTextField textField = new ResourcefulTextField(resource);
    ...
    /**
     * Constructs a TextField bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulTextField(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.text.field", ex.getMessage(), ex);
        }
    }
