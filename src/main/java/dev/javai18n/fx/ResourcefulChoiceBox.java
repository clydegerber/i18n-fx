...
/**
 * A ChoiceBox that supports localizing the name (id), style, item values,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulChoiceBox extends ChoiceBox<String> implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulChoiceBox with the specified Resource.
     * @param resource A Resource containing a ChoiceBoxPropertyBundle.
     * @return A ResourcefulChoiceBox with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulChoiceBox create(Resource resource)
    {
        ResourcefulChoiceBox choiceBox = new ResourcefulChoiceBox(resource);
    ...
    /**
     * Constructs a ChoiceBox bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulChoiceBox(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            ChoiceBoxPropertyBundle props = (ChoiceBoxPropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.choice.box", ex.getMessage(), ex);
        }
    }
