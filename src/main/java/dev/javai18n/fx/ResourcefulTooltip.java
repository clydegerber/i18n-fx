 ...
 */
public class ResourcefulTooltip extends Tooltip implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTooltip with the specified Resource.
     *
     * @param resource A Resource containing a TooltipPropertyBundle.
     * @return A ResourcefulTooltip with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTooltip create(Resource resource)
    {
        ResourcefulTooltip tooltip = new ResourcefulTooltip(resource);
        tooltip.initialize();
        return tooltip;
    }
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            TooltipPropertyBundle props = (TooltipPropertyBundle) getResource().getObject();
            String text = props.getText(); if (null != text) setText(text);
        }
        catch (MissingResourceException | ClassCastException ex)
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.tooltip", ex.getMessage(), ex);
        }
    }
