...
/**
 * An HTMLEditor that supports localizing the name (id), style,
 * accessible text, accessible help, and accessible role description.
 *
 * <p>Note: HTMLEditor extends {@link javafx.scene.layout.Region} (not Control),
 * so it does not have a {@code setTooltip()} method. Tooltip support is therefore
 * not provided by this class. Use a {@link NodePropertyBundle} to supply localizable
 * properties for an HTMLEditor.</p>
 */
public class ResourcefulHTMLEditor extends HTMLEditor implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulHTMLEditor with the specified Resource.
     * @param resource A Resource containing a NodePropertyBundle.
     * @return A ResourcefulHTMLEditor with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulHTMLEditor create(Resource resource)
    {
        ResourcefulHTMLEditor htmlEditor = new ResourcefulHTMLEditor(resource);
    ...
    /**
     * Constructs an HTMLEditor bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulHTMLEditor(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            NodePropertyBundle props = (NodePropertyBundle) getResource().getObject();
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.html.editor", ex.getMessage(), ex);
        }
    }
