...
/**
 * A TextArea that supports localizing the name (id), style, prompt text, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulTextArea extends TextArea implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTextArea with the specified Resource.
     * @param resource A Resource containing a TextInputPropertyBundle.
     * @return A ResourcefulTextArea with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTextArea create(Resource resource)
    {
        ResourcefulTextArea textArea = new ResourcefulTextArea(resource);
        textArea.initialize();
        return textArea;
    }
    ...
    /**
     * Constructs a TextArea bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulTextArea(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.text.area", ex.getMessage(), ex);
        }
    }
