...
/**
 * A ComboBox that supports localizing the name (id), style, prompt text, item values, tooltip,
 * accessible text, accessible help, and accessible role description.
 *
 * <p>When the item values are updated on a locale change, the current selection is preserved
 * by index: if the selected index is still valid after the new items are set, the same index
 * is re-selected.  This means the selection tracks the item at the same position in the list,
 * not the same logical value — if the locale bundle changes item count or ordering, the
 * selected item may differ.</p>
 */
public class ResourcefulComboBox extends ComboBox<String> implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulComboBox with the specified Resource.
     * @param resource A Resource containing a ComboBoxPropertyBundle.
     * @return A ResourcefulComboBox with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulComboBox create(Resource resource)
    {
        ResourcefulComboBox comboBox = new ResourcefulComboBox(resource);
        comboBox.initialize();
        return comboBox;
    }
    ...
    protected void updateLocaleSpecificValues()
    {
        try
        {
            ComboBoxPropertyBundle props = (ComboBoxPropertyBundle) getResource().getObject();
            ...
            if (null != values)
            {
                ...
                getItems().setAll(Arrays.asList(values));
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.combo.box", ex.getMessage(), ex);
        }
    }
