...
/**
 * A TabPane that supports localizing the name (id), style, tooltip,
 * accessible text, accessible help, and accessible role description.
 */
public class ResourcefulTabPane extends TabPane implements Resourceful, LocaleEventListener
{
    static
    {
    ...
    /**
     * Construct a ResourcefulTabPane with the specified Resource.
     * @param resource A Resource containing a ControlPropertyBundle.
     * @return A ResourcefulTabPane with updated Locale-specific values that is registered
     *         to listen to LocaleEvents generated from the specified Resource's source.
     */
    public static ResourcefulTabPane create(Resource resource)
    {
        ResourcefulTabPane tabPane = new ResourcefulTabPane(resource);
        tabPane.initialize();
        return tabPane;
    }
    ...
    /**
     * Constructs a TabPane bound to the given resource. Use {@link #create(Resource)}
     * for an initialized instance.
     * @param resource The resource identifying the locale source and bundle key.
     */
    protected ResourcefulTabPane(Resource resource)
    {
    ...
    protected void updateLocaleSpecificValues()
    {
        ...
        {
            FX_LOGGER.log(System.Logger.Level.WARNING, "missing.resource.for.tab.pane", ex.getMessage(), ex);
        }
    }
