...
/**
 * Unit tests for ResourcefulMenu.
 */
public class TestResourcefulMenu
{
    @BeforeAll
    static void init()
    {
    ...
    void testInitialProperties() throws Exception
    {
        FXTestHelper.runAndWait(() ->
        {
            ...
            ResourcefulMenu menu = ResourcefulMenu.create(new Resource(source, "TestMenu"));
    ...
    {
        TestComponentSource source = TestComponentSource.create();
        ResourcefulMenu[] holder = new ResourcefulMenu[1];
        FXTestHelper.runAndWait(() ->
        {
            holder[0] = ResourcefulMenu.create(new Resource(source, "TestMenu"));
        });
        source.setBundleLocale(Locale.FRANCE);
        FXTestHelper.runAndWait(() -> {}); // drain FX queue
        assertEquals("Menu Test", holder[0].getText());
    }
}
