 ...
 */
public final class FXTestModuleRegistrar
{
    private static volatile boolean registered;

    private FXTestModuleRegistrar() {}

    /**
     * Ensure that all module-level registrations for the i18n-fx test module
    ...
    public static void ensureRegistered()
    {
        if (!registered)
        {
            synchronized (FXTestModuleRegistrar.class)
            {
                if (!registered)
                {
                    ...
                    AttributeCollectionResourceBundle.registerAttributeCollectionPackage(
                        NodePropertyBundle.class.getPackageName());
                    registered = true;
                }
            }
        }
